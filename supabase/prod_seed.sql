-- Production seed for Snapt (extracted from seed.sql lines 1-1727 on 2026-07-29).
-- Contains ONLY business config (app_config) and the 13 published policy
-- documents. NO demo accounts, creators, admins, or consents — those are
-- local-only. Run ONCE against a freshly migrated database; the inserts are
-- not idempotent.
...
insert into app_config (key, value, description, confirmed) values
  ...
  ('portfolio_preapproval_count', '3', 'First N portfolio submissions need moderator approval; auto-publish after (default 3 — confirm with Don)', true),
